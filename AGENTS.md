...
このファイルは x-true-block-mute リポジトリの共通運用ルールです。Codex の自律開発では現行のユーザー指示とハンドオフを優先し、本ファイルはデータ保護・権限境界・検証報告の不変条件を補完します。旧 ChatGPT 承認制は廃止済みです。現行のユーザー指示で許可された自律開発の範囲では、Codex / Claude Code が通常の docs・test・code 健全性タスクを進めます。権限追加、Phase 移行、配布、外部送信などの製品・公開・データ境界は引き続き人間承認ゲートです。

### 承認と進め方

- ユーザー向け報告は日本語を主にし、冒頭に日本時間 `YYYY/MM/DD HH:MM:SS` を付ける。
- タスクは現行ユーザー指示と本リポジトリの人間承認ゲートに従って進める。通常の docs・test・code 健全性作業は自走し、大きな方針変更（権限追加、Phase 移行、配布、外部送信）は着手前にユーザー承認を取る。
