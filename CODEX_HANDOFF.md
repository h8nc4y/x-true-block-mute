...
- final private-marker gateではHEADのtracked内容に既存11件、変更9ファイルの隔離tracked内容に追加0件を確認した。Projects workspace root直下の隔離用一時tree `.scan-tmp-012-20260729-1536` は通常の再帰削除が実行前に安全ポリシーで拒否された後、固定絶対path・workspace内・205ファイルを確認した上でWindows Recycle Binへ移動し、元pathの不存在を確認した。追加のcleanup／復元は行わず、同じ代替手段を今後の既定にしない。
