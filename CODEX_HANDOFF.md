...
## 4. リポジトリの現状（2026-06-29 14:50 JST 時点）
...
- 2026-06-29 14:50 JST 時点の `main` 最新は PR #20 merge commit `cade613`（GitHub PR #20 mergedAt `2026-06-28T15:32:12Z`）。PR #20 は PR #19 後のハンドオフ同期で、open PR は 0。`PHASE2-HOOK-PRODUCTION` では off-settings XHR の body 非読取と、`SyncCapture` 未注入時の retry 可能性をローカルテストで固定済み。明示 teardown は watch-item だが launch blocker ではない。
