...
- `sync/sync-hook.js` — MAIN フック。private `installedHook && active` と既存APIのactive wrapper ownershipを正本として `fetch` / `XHR` を冪等ラップし、公開 `window.__xTbmSyncHookInstalled` は自己修復するmirrorとして扱う。XHR の次request stateはdelegate中に公開せず、`open()` 正常復帰後だけ有効化し、外部wrapperの同期DONE／throwをfail closedに扱う。install世代共有のper-XHR coordinatorはactive／inactive wrapperのdelegation depth、tree token、`delegating` / `committed` phaseを持ち、depth内のnested open tree全体、commit後のinactive旧wrapper直呼び、同じinactive wrapperの二重通過、retained inactive ancestor配下のactive nested openで委譲順が曖昧なstateを無効化する。ambiguous treeはinner / outerともcommitせず、unwind後の独立top-level openだけ再armする。`fetch` は初回script評価時の標準`Request.prototype.url` getterと基準fetchを固定し、後から重なった外部wrapper配下ではinactive旧hookが観測した実委譲入力のidentityと返却Promiseが現世代の外側呼出しと同一で、かつ各旧世代自身のdelegateも基準fetchまで再帰的に証明できる直列委譲だけを処理する。inactive APIを残したscript再評価は同じclosureを再利用し、foreign wrapperを新しい基準fetchへ昇格させない。置換・分岐・Promise差替え・内側世代のprovenance不明・観測不能は元Promiseを保って本文未読／message未送信へ倒す。cursor を持たない初回 list request の正常応答では、request variables を含めず固定の `sync-start` を entries より先に同一オリジンへ postMessage する。
...
- 2026-07-29 に同じ bounded review を継続し、同じ宣言的scriptが同一documentで再評価されると2回目が公開APIだけを上書きし、初回hookをteardownできず、続く再installで本文読取とmessageが各2回になるREDをsynthetic再現した。稼働中は既存APIとwrapper所有権を正本として再評価を終了し、既存APIのuninstallが元の `fetch` / `XMLHttpRequest.open` を復元でき、再install後も各1回だけ処理する回帰を固定した（PR #53、merge `49a7c61`）。exact merge commitでhook 101件と静的10本をPASS。GitHub check rollupとbranch workflow runは各0件でremote CI証跡はない。権限・endpoint・storage schema・実データ境界は変更していない。
- 2026-07-29 に同じ bounded review を継続し、次のeligible XHR stateをdelegate前に公開すると、外部 `open` wrapperのpre-delegate DONEが旧non-list本文を新URLとして読み、delegate後DONE→同期throwでもcatch前にmessageを送るREDをsynthetic再現した。委譲中は旧／次stateを無効化し、`originalOpen` 正常復帰後だけ次stateをcommitする。委譲後・return前の同期DONEはcommit後に再確認して1回処理し、throw時は本文・messageを0件に固定した。独立レビューで同期DONE中の再入openをpost-delegate / pre-delegateの両順序、uninstall / reinstall、現世代commit後のinactive旧wrapper直呼び、delegation中の同一inactive wrapper二重通過、未復帰active／retained inactive outer配下でinnerがpost-delegate DONEまで正常完了する経路と組み合わせ、単純なlatest-state保持が誤対応するREDを確認した。install世代共有のper-XHR coordinatorでactive／inactive wrapperのdelegationを `try/finally` でdepthへ含め、`depth > 0` 中のnested open tree全体をambiguous化してtokenを消し、inner / outerともcommit・即時処理を禁止する。tree unwind後の独立top-level openだけ再armするため、曖昧なtree内の有効responseを落とし得るavailability tradeoffを安全側に採用した。`verify-sync-hook.mjs` 138件をPASS。権限・endpoint・storage schema・実データ境界は変更していない。
- PR #55 時点の private-marker gate にある「HEAD tracked 11件」は1件のunder-countだった。2026-07-29 のread-only比較で、追加扱いされていた `tests/scripts/verify-f1a-observation-safety.mjs` の該当行はbase `2ad31a1` と同一で、該当file・scanner・runnerもbaseから現HEADまでdiff 0と確認したため、当時の正しい既存baselineは12件。値はすべて非開示とし、変更ファイルの隔離scanではこのbaselineとの差分だけを判定する。PR #55 の隔離scanで使った一時treeはcleanup済みだが、通常の再帰削除がpolicy拒否された後の代替cleanupを今後の既定にしない。
- このXHR follow-upはPR #55（head `a7e6a2a`、merge `596c4e2`）でmainへ統合済み。exact merge commitの静的10本はPASS（941 ms）、PR checkとhead commitのworkflow runは各0件でコードテストのremote CI証跡はない。main merge commitのGitHub Pages build/deploy run `30429121391` はsuccessで、Pages statusはbuilt、公開URLのHEADはHTTP 200を確認した。
- 2026-07-29 の次の bounded review では、公開installed flagがfalseへdriftした状態で同じAPIを再installするとfetch / XHRを二重wrapし、script再評価でも初回APIを上書きしてnative teardown所有権を失うREDをsynthetic再現した。private `installedHook && active` と既存APIのactive wrapper ownershipを正本にし、公開flagはmirrorとして自己修復する。private stateが無いのに公開flagだけtrueのfresh contextはinstallを妨げない。targeted hook 148件と静的10本をPASSし、新権限・endpoint・storage schema・live X・raw responseは不使用。PR #57（head `05d80d8`、merge `7407fbf`）でmainへ統合し、exact merge commitでも静的10本をPASSした。PR check rollupは0件でコードテストCI証跡はなく、自動GitHub Pages run `30431664453` のsuccessはコードテストの代替にしない。
- 2026-07-30 の bounded review では、page / 外部wrapperが所有する fetch input の `url` getterを、型判定と値取得で2回評価する境界をsynthetic TDDで再現した。1回目と2回目でURLが変わると別endpointとして分類され、2回目だけthrowするとnative fetch開始後にcallerへhook由来の同期errorを追加していた。URLを1回だけsnapshotし、取得不能時は元fetch resultをそのまま返して本文未読・message未送信へfail closedにした。独立レビューでは、実`Request`のown getterが標準`url`をshadowすると、native fetchが使う内部HomeTimeline URLとhookが見る偽BlockedAccounts URLを食い違わせられるP1を検出した。標準`Request.prototype.url` getterから得るbrowser-owned URLとの完全一致を追加し、不一致・brand不明・getter取得不能を本文未読・message未送信へ閉じた。次のレビューでは、初回install後にpage側がprototype getterを偽値へ差し替え、公開APIでuninstall / reinstallすると、installごとの再取得が偽getterを信頼して一致判定を迂回する追加P1を検出した。標準getterをscript評価時のclosureへ一度だけ固定し、prototype drift後の再installでも内部HomeTimeline本文とmessageを0件に保つ。追加レビューでは、外部wrapperがeligibleなRequest／stringを内部HomeTimeline入力へ置換してinactive旧hookへ渡した後、現世代が外側URLだけで返却responseを分類するP1をRED再現した。外部wrapper配下はinstall世代共有の同期traceで、inactive旧hookが観測した実入力identityと返却Promiseが外側呼出しと一致する直列委譲だけを証明済みとし、置換・複数枝・Promise差替え・観測不能を本文未読／message未送信へ閉じる。最終の独立レビューでは、opaque delegateを捕捉した旧世代をtransparent wrapper経由で次世代が呼ぶと、旧世代入口の一致だけで内側の未証明delegateをlaunderできるP1と、foreign wrapper保持中のuninstall後にscriptを再評価すると現在のwrapperを新closureが基準fetchとして信頼するP1を検出した。各inactive世代は自分のdelegateも基準fetchまで再帰検証してからancestor proofへ参加し、inactive旧APIが残る再評価は同じclosureを再利用する。続くP2では、公開APIのshape getter／Proxyがthrowするとscript評価を止め、shape一致のno-op APIがfresh installを妨げる境界を検出した。API objectとmethod getterのprobe全体を例外隔離し、captured methodのinstall後に同一API identityとprivate active ownershipを再確認できない場合は、現在のfetchを基準扱いしないfresh fail-closedへ進む。3世代laundering、script再評価、throwing API getter、no-op APIを含む `verify-sync-hook.mjs` 204 checksと静的10本をPASS。変更worktreeとbase mainのtracked private-markerは現行scannerで双方11件（値非開示、差分0）、Gitleaksはworking tree／108 commitsで0件、Semgrepは68 rules／2 targetsで0件、独立レビューはP0–P3なしだった。新権限・endpoint・storage schema・live X・raw response・製品UIは変更していない。
- このfetch URL provenance hardeningはPR #59（head `0e7f109`、merge `4138d63`）でmainへ統合済み。PR check rollupとbranch workflow runは各0件でremote code-test証跡はない。exact merge commitの `node scripts/check-all.mjs` は静的10本（hook 204 checksを含む）をPASSし、main／origin/main／remote main一致、clean、task branch／worktree cleanup完了を確認した。
...
4. **`PHASE2-HOOK-PRODUCTION` の bounded review** — MAIN-world hook の lifecycle / teardown / idempotency はローカル fixture と静的レビューだけで扱う。明示 teardown、in-flight停止、再 install、同一documentでのscript再評価時のAPI/wrapper所有権、fetch input URL accessorのsingle-read、初回script評価時に固定した標準getterによるbrowser-owned `Request`内部URL一致、外部fetch wrapper配下で各inactive旧hookが再帰検証する実入力identity／返却Promiseの直列委譲証明、inactive APIを残すscript再評価時のclosure再利用、外部 `open` wrapperの同期DONE／throwに対する正常復帰後のstate commit境界、install世代共有coordinatorによるdelegation treeとinactive旧wrapper bypassのambiguous-state無効化、tail 厳格化（PR #34）、安全な全走査開始検出（PR #41）、exact GraphQL endpoint pathname gate（2026-07-25）は固定済み。今後も新権限・新データソース・raw response 取得・live X 読み取りはしない。
