...
- `sync/sync-hook.js` — MAIN フック。private `installedHook && active` と既存APIのactive wrapper ownershipを正本として `fetch` / `XHR` を冪等ラップし、公開 `window.__xTbmSyncHookInstalled` は自己修復するmirrorとして扱う。XHR の次request stateはdelegate中に公開せず、`open()` 正常復帰後だけ有効化し、外部wrapperの同期DONE／throwをfail closedに扱う。install世代共有のper-XHR coordinatorはactive／inactive wrapperのdelegation depth、tree token、`delegating` / `committed` phaseを持ち、depth内のnested open tree全体、commit後のinactive旧wrapper直呼び、同じinactive wrapperの二重通過、retained inactive ancestor配下のactive nested openで委譲順が曖昧なstateを無効化する。ambiguous treeはinner / outerともcommitせず、unwind後の独立top-level openだけ再armする。cursor を持たない初回 list request の正常応答では、request variables を含めず固定の `sync-start` を entries より先に同一オリジンへ postMessage する。
...
4. **`PHASE2-HOOK-PRODUCTION` の bounded review** — MAIN-world hook の lifecycle / teardown / idempotency はローカル fixture と静的レビューだけで扱う。明示 teardown、in-flight停止、再 install、同一documentでのscript再評価時のAPI/wrapper所有権、外部 `open` wrapperの同期DONE／throwに対する正常復帰後のstate commit境界、install世代共有coordinatorによるdelegation treeとinactive旧wrapper bypassのambiguous-state無効化、tail 厳格化（PR #34）、安全な全走査開始検出（PR #41）、exact GraphQL endpoint pathname gate（2026-07-25）は固定済み。今後も新権限・新データソース・raw response 取得・live X 読み取りはしない。
