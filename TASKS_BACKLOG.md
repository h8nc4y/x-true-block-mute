...
| --- | --- | --- | --- | --- | --- | --- |
| P2-001 | M1 | backlog 再編（本ファイル更新） | 高 | S | doing | - |
| P2-002 | M1 | ガバナンス文書改訂（AGENTS / README / gates / threat model / deferred register / research docs） | 高 | M | ready | P2-001 |
| P2-003 | M1 | 検証スクリプト lockstep 更新（verify-docs-consistency / audit-operational-alignment） | 高 | S | ready | P2-002 と同一コミット |
| P2-004 | M2 | CDP 自動検証スクリプト `tests/scripts/verify-extension-load-chrome.mjs`（依存ゼロ、Playwright キャッシュ Chromium 使用） | 高 | M | ready | P2-003 |
| P2-005 | M3 | F1-A live masked summary 収集と `evaluate-f1-observation.mjs --live` 判定 | 高 | M | ready | P2-004 |
| P2-006 | M3 | `docs/decisions/f1-source-selection.md` の decision 確定（viable/insufficient の記録） | 高 | S | ready | P2-005 |
| P2-007 | M4 | 宣言的 MAIN world `content_scripts` 移行検証（`scripting` permission 削除可否） | 高 | S | ready | P2-005 |
| P2-008 | M4 | production capture script（user_id / handle / listKind のみ抽出） | 高 | M | ready | P2-006, P2-007 |
| P2-009 | M4 | storage schema v2（upsert / dedupe / listKind / syncedAt / stale 削除 / migration） | 高 | M | ready | P2-006 |
| P2-010 | M4 | popup 同期導線（同期ボタン・進捗・最終同期時刻・削除） | 高 | M | ready | P2-008, P2-009 |
...
| P2-016 | M6 | options page（entries 管理・プライバシー説明）＋research UI の本番非表示化 | 中 | M | ready | P2-010 |
...
| TB-002 | Chrome Load unpacked と popup の確認 | skip: 人間確認が必要 | ready → P2-004 に統合。Codex 失敗の根本原因は branded Chrome 137+ の `--load-extension` 無効化と推定。Claude Code が CDP 自動検証で実施。 |
