...
| P2-009 | M4 | storage schema v2（upsert / dedupe / listKind / syncedAt / stale 削除 / migration） | 高 | M | doing: 基盤＋sync-state（enabled/lastSyncedAt）done、追加 upsert・dedupe は live 実証済み。stale reconciliation（全置換・完了検出）が残り。 | source 非依存のため先行実装 |
| P2-010 | M4 | popup 同期導線（同期ボタン・進捗・最終同期時刻・削除） | 高 | M | done: 「ブロック・ミュート同期」セクション（有効化トグル・件数・最終同期・設定ページリンク・削除）。M2 で実機検証。popup の onChanged 自動更新は M6 polish。 | P2-008, P2-009 |
| P2-009b | M4 | stale reconciliation（完了検出＝末尾カーソル到達時に全置換、部分時は追加のみ） | 高 | M | ready: ユーザー選択は「全置換（完全同期時のみ）」。 | P2-008 |
