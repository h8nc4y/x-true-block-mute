...
このファイルは現行タスクのトラッカーです。Codex は現行のユーザー指示とハンドオフを優先し、ここではロードマップとタスク状態を実装実態に合わせて記録します。2026-06-13 のガバナンス変更により、旧 ChatGPT 承認制は廃止され、ユーザー本人がチャットで直接承認したタスクを Claude Code が実装します。
