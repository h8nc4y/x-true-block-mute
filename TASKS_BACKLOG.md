...
- 2026-07-29: `POST-2026-07-29-SYNC-HOOK-DOUBLE-EVALUATION` を Class M のsynthetic-only TDDで完了。同一documentでscriptを2回評価すると公開APIが初回hookのteardown所有権を失い、再install後に本文読取/messageが各2回になるREDを固定した。稼働中の既存APIとwrapperを再評価時も保持し、uninstall復元と再install後の1回処理を回帰化した。
