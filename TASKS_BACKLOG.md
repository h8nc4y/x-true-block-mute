...
| P2-009 | M4 | storage schema v2（upsert / dedupe / listKind / syncedAt / stale 削除 / migration） | 高 | M | doing: 基盤（listKind/syncedAt フィールド、`upsertSyncedEntries`/`clearSyncedEntries`、user_id 優先 dedupe、migration、テスト）done。stale reconciliation は full-list 取得可否（M3 pagination）に依存するため M3 後に実装。 | source 非依存のため先行実装 |
