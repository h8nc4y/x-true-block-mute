...
| REVIEW-2026-07-05-SYNC-COMPLETE | Sync completion / reconcile robustness (PHASE2-HOOK-PRODUCTION follow-up) | Confirmed by the 2026-07-05 multi-agent review. `hasBottomCursor`-based completion (`sync-capture.js` / `sync-hook.js`) treats any 0-user page that carries a Bottom cursor as the list tail, so a mid-list page with a Bottom cursor but no extractable users (e.g. a contiguous run of suspended accounts, or an intermediate loading page) can fire `sync-complete` prematurely; the bridge then reconciles the synced set down to the partial staging, silently dropping still-blocked/muted accounts from the filter until the true tail is reached (recoverable by scrolling to the real end or clear+resync). NOT fixed autonomously: the completion contract was established through live validation, and the correct hardening depends on X's real terminal-page shape, which is 未確認. | The safe fix (e.g. "complete only when the page has a Bottom cursor AND zero non-cursor timeline entries", and/or a reconcile guard) changes a live-validated, privacy-critical path; shipping it blind could regress the un-block reconcile. | A masked live re-verification (Chrome MCP, owner consent — masked observation only, no raw values) characterizing X's terminal-page shape for BlockedAccounts/MutedAccounts, then a scoped fix with a synthetic-fixture regression test. No new permissions, no raw response handling. |
