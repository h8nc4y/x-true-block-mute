...
- A next XHR request state becomes readable only after delegated `open()` returns normally. Synchronous DONE events emitted inside a foreign wrapper are ignored while delegation is ambiguous, then rechecked once after a successful return; a wrapper that throws after DONE cannot expose a provisional body or post a message. A per-XHR coordinator is shared across hook install generations and records active/inactive-wrapper delegation depth, a tree token, ambiguity, and explicit `delegating` / `committed` phases. Every retained inactive wrapper counts its delegated `originalOpen` in the same depth with `try/finally`, so an active-generation nested open beneath either an active or inactive ancestor invalidates the whole tree before an inner synchronous DONE can commit or post. The tree stays disarmed through every inner/outer return and rearms only on a later independent top-level open. This intentionally trades availability for privacy/correct association: an otherwise usable response inside an ambiguous tree can be dropped. A direct call to an inactive retained wrapper after commit also invalidates the token before delegation, while repeated traversal of the same inactive wrapper during one tree fails closed.
